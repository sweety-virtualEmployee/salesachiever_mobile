...
class PotentialInfoScreen extends StatefulWidget {
  ...
  @override
  _PotentialInfoScreenState createState() => _PotentialInfoScreenState();
}

class _PotentialInfoScreenState extends State<PotentialInfoScreen> {
  dynamic _deal;
  bool _readonly = true;
  bool _isValid = false;
  String _dealTypeId = '';
  ...
  _initialize() {
    var userFields = PotentialService().getuserFields();
  ...
  _validate() {
    var isValid = PotentialService().validateEntity(_deal);
  ...
  onSave() async {
    ...
    try {
      ...
      if (_deal['DEAL_ID'] != null) {
        await PotentialService().updateEntity(_deal!['ACCT_ID'], _deal);
      } else {
        var newEntity = await PotentialService().addNewEntity(_deal);
        _deal['ACCT_ID'] = newEntity['ACCT_ID'];
      }
