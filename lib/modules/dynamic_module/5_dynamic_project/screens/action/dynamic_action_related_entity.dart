...
class DynamicActionRelatedEntityScreen extends StatefulWidget {
  const DynamicActionRelatedEntityScreen({
    Key? key,
  ...
  @override
  _DynamicActionRelatedEntityScreenState createState() =>
      _DynamicActionRelatedEntityScreenState();
}

class _DynamicActionRelatedEntityScreenState
    extends State<DynamicActionRelatedEntityScreen> {
  List<dynamic> list = [];
        ...
        builder: (context, provider, child) {
          return PsaScaffold(
            action: (widget.type == 'company' ||
                ...
                ? PsaAddButton(
              onTap: () async {
                if (provider.getActionEntity['DEAL_ID'] != null) {
                  await Navigator.push(
                    context,
                    platformPageRoute(
                      context: context,
                      builder: (BuildContext context) => AddRelatedEntityScreen(
                        multiId: provider.getActionEntity['MULTI_ID'],
                        deal: {
                          'ID': provider.getActionEntity['DEAL_ID'],
                          'TEXT': provider.getActionEntity['DESCRIPTION'],
                        },
                        account: provider.getActionEntity['ACCT_ID'] != null
                            ? {
                          'ID': provider.getActionEntity['ACCT_ID'],
                          'TEXT': provider.getActionEntity['ACCTNAME'],
                        }
                            : {},
                        contact: provider.getActionEntity['CONT_ID'] != null
                            ? {
                          'ID': provider.getActionEntity['CONT_ID'],
                          'TEXT': '${provider.getActionEntity['FIRSTNAME'] ?? ''} ${provider.getActionEntity['SURNAME'] ?? ''}',
                        }
                            : {},
                        type: "",
                      ),
                  ...
                  await Navigator.push(
                    ...
                    platformPageRoute(
                      ...
                      builder: (BuildContext context) => AddRelatedEntityScreen(
                        project: {
                          'ID': provider.getActionEntity['PROJECT_ID'],
                          'TEXT': provider.getActionEntity['PROJECT_TITLE'],
                        },
                        account: {
                          'ID': provider.getActionEntity['ACCT_ID'],
                          'TEXT': provider.getActionEntity['ACCTNAME'],
                        ...
                        contact: {
                          'ID': provider.getActionEntity['CONT_ID'],
                          'TEXT': provider.getActionEntity['CONT_ID'] !=
                              null
                              ? '${provider.getActionEntity['FIRSTNAME'] ??
                              ''} ${provider.getActionEntity['SURNAME'] ?? ''}'
                              : null,
                        },
                      ),
                    ),
                    ...
                    var result = await CompanyService().getRelatedEntity(
                      'project',
                      provider.getActionEntity["PROJECT_ID"],
                      'companies',
                    );
                ...
                : ((widget.type == 'projects' && provider.getActionEntity['CONT_ID'] == null) && (widget.entityType != "ACTION"))
                ? PsaAddButton(
              onTap: () async {
                if (provider.getActionEntity['ACCT_ID'] != null) {
                  ...
                  if (provider.getActionEntity['DEAL_ID'] != null) {
                    var project = await DynamicProjectService()
                        .getEntityById(
                        "OPPORTUNITY", provider.getActionEntity['DEAL_ID']);
                    print(
                        "project data value get${project.data.toString()}");
                    print(project.data["DEAL_NAME"]);
                    setState(() {
                      provider.getActionEntity["DEAL_NAME"] =
                    ...
                    platformPageRoute(
                      ...
                      builder: (BuildContext context) =>
                          AddRelatedEntityScreen(
                            ...
                            deal: {
                              'ID': provider.getActionEntity['DEAL_ID'],
                              'TEXT': provider.getActionEntity['DEAL_NAME'],
                            },
                            type: "opp",
                          ),
                    ),
              ...
              onTap: () async {
                ...
                ? PsaAddButton(
              onTap: () async {
                if (provider.getActionEntity['MULTI_ID'] == null &&
                    provider.getActionEntity['PROJECT_ID'] != null) {
                  ...
                  print("result${result}");
                  dynamic value = {
                    "PROJECT_ID": provider.getActionEntity['PROJECT_ID']
                  };
                  print(value);
                  ...
                  if (provider.getActionEntity['ACCT_ID'] != null) {
                    ...
                    await DynamicProjectService()
                        ...
                        provider.getActionEntity['ACCT_ID']);
                    print(
                        "project data value get${project.data}");
                    print(project.data["ACCTNAME"]);
                    setState(() {
                      provider.getActionEntity["ACCTNAME"] =
                      ...
                      builder: (BuildContext context) =>
                          AddRelatedEntityScreen(
                            ...
                            project: {
                              'ID': provider.getActionEntity['PROJECT_ID'],
                              'TEXT':
                              provider.getActionEntity['PROJECT_TITLE'],
                            },
                            account: provider.getActionEntity['ACCT_ID'] !=
                                null
                                ? {
                              'ID':
                              provider.getActionEntity['ACCT_ID'],
                              ...
                              provider.getActionEntity['ACCTNAME'],
                                ...
                                : {},
                            contact: provider.getActionEntity['CONT_ID'] !=
                                null
                                ? {
                              'ID':
                              provider.getActionEntity['CONT_ID'],
                              'TEXT':
                              '${provider.getActionEntity['FIRSTNAME'] ?? ''} ${provider.getActionEntity['SURNAME'] ?? ''}',
                            }
                                : {},
                            type: "opp",
                          ),
    ...
    if (widget.type == "companies") {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) {
            return DynamicProjectNotes(
              project: _dynamicTabProvider.getActionEntity,
              typeNote: widget.type,
              notesData: response,
              isNewNote: false,
              entityType: widget.entityType,
  ...
  Future<void> handleOnTapFunction(dynamic item) async {
      ...
      if (widget.type == "companies") {
        ...
        print(
            "deal id valeudifgc${_dynamicTabProvider.getActionEntity['DEAL_DEAL_ID']}");
        dynamic deal = await OpportunityService()
            .getEntity(item['DEAL_ID'].toString());
        Navigator.push(
          context,
