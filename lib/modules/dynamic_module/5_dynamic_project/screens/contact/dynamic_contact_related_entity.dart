...
class DynamicContactRelatedEntityScreen extends StatefulWidget {
  const DynamicContactRelatedEntityScreen({
    Key? key,
  ...
  @override
  _DynamicContactRelatedEntityScreenState createState() =>
      _DynamicContactRelatedEntityScreenState();
}

class _DynamicContactRelatedEntityScreenState
    extends State<DynamicContactRelatedEntityScreen> {
  List<dynamic> list = [];
  ...
  callApi() async {
    ...
    if (widget.entity?["ACCT_ID"]!=null) {
      ...
      _dynamicTabProvider.getContactEntity["ACCTNAME"] = entity.data["ACCTNAME"];
    }
    if (widget.entity?["PROJECT_ID"]!=null) {
      var entity = await ProjectService().getEntity(widget.entity?["PROJECT_ID"]);
      _dynamicTabProvider.getContactEntity["PROJECT_TITLE"] = entity.data["PROJECT_TITLE"];
    }
    if (widget.entity?["CONT_ID"]!=null) {
      var entity = await ContactService().getEntity(widget.entity?["CONT_ID"]);
      _dynamicTabProvider.getContactEntity["FIRSTNAME"] = entity.data["FIRSTNAME"];
    }
    if (widget.entity?["DEAL_ID"]!=null) {
      var entity = await OpportunityService().getEntity(widget.entity?["DEAL_ID"]);
      _dynamicTabProvider.getContactEntity["DESCRIPTION"] = entity.data["DESCRIPTION"];
    }
    _dynamicTabProvider.setContactEntity(_dynamicTabProvider.getContactEntity);
        ...
        builder: (context, provider, child) {
          return PsaScaffold(
            action: (widget.type == 'company' ||
                ...
                ? PsaAddButton(
              onTap: () async {
                if (provider.getContactEntity['DEAL_ID'] != null) {
                  await Navigator.push(
                    context,
                    platformPageRoute(
                      context: context,
                      builder: (BuildContext context) => AddRelatedEntityScreen(
                        multiId: provider.getContactEntity['MULTI_ID'],
                        deal: {
                          'ID': provider.getContactEntity['DEAL_ID'],
                          'TEXT': provider.getContactEntity['DESCRIPTION'],
                        },
                        account: provider.getContactEntity['ACCT_ID'] != null
                            ? {
                          'ID': provider.getContactEntity['ACCT_ID'],
                          'TEXT': provider.getContactEntity['ACCTNAME'],
                        }
                            : {},
                        contact: provider.getContactEntity['CONT_ID'] != null
                            ? {
                          'ID': provider.getContactEntity['CONT_ID'],
                          'TEXT': '${provider.getContactEntity['FIRSTNAME'] ?? ''} ${provider.getContactEntity['SURNAME'] ?? ''}',
                        }
                            : {},
                        type: "",
                      ),
                  ...
                  await Navigator.push(
                    ...
                    platformPageRoute(
                      ...
                      builder: (BuildContext context) => AddRelatedEntityScreen(
                        project: {
                          'ID': provider.getContactEntity['PROJECT_ID'],
                          'TEXT': provider.getContactEntity['PROJECT_TITLE'],
                        },
                        account: {
                          'ID': provider.getContactEntity['ACCT_ID'],
                          'TEXT': provider.getContactEntity['ACCTNAME'],
                        ...
                        contact: {
                          'ID': provider.getContactEntity['CONT_ID'],
                          'TEXT': provider.getContactEntity['CONT_ID'] !=
                              null
                              ? '${provider.getContactEntity['FIRSTNAME'] ??
                              ''} ${provider.getContactEntity['SURNAME'] ?? ''}'
                              : null,
                        },
                      ),
                    ),
                    ...
                    var result = await CompanyService().getRelatedEntity(
                      'project',
                      provider.getContactEntity["PROJECT_ID"],
                      'companies',
                    );
                ...
                : ((widget.type == 'projects' && provider.getContactEntity['CONT_ID'] == null) && (widget.entityType != "ACTION"))
                ? PsaAddButton(
              onTap: () async {
                if (provider.getContactEntity['ACCT_ID'] != null) {
                  ...
                  if (provider.getContactEntity['DEAL_ID'] != null) {
                    var project = await DynamicProjectService()
                        .getEntityById(
                        "OPPORTUNITY", provider.getContactEntity['DEAL_ID']);
                    print(
                        "project data value get${project.data.toString()}");
                    print(project.data["DEAL_NAME"]);
                    setState(() {
                      provider.getContactEntity["DEAL_NAME"] =
                    ...
                    platformPageRoute(
                      ...
                      builder: (BuildContext context) =>
                          AddRelatedEntityScreen(
                            ...
                            deal: {
                              'ID': provider.getContactEntity['DEAL_ID'],
                              'TEXT': provider.getContactEntity['DEAL_NAME'],
                            },
                            type: "opp",
                          ),
                    ),
              ...
              onTap: () async {
                      ...
                      platformPageRoute(
                        ...
                        builder: (BuildContext context) =>
                            DynamicActionTypeScreen(
                              action: {"CONT_ID": widget.entity?["CONT_ID"]},
                              popScreens: 2,
                ...
                ? PsaAddButton(
              onTap: () async {
                if (provider.getContactEntity['MULTI_ID'] == null &&
                    provider.getContactEntity['PROJECT_ID'] != null) {
                  ...
                  print("result${result}");
                  dynamic value = {
                    "PROJECT_ID": provider.getContactEntity['PROJECT_ID']
                  };
                  print(value);
                  ...
                  if (provider.getContactEntity['ACCT_ID'] != null) {
                    ...
                    await DynamicProjectService()
                        ...
                        provider.getContactEntity['ACCT_ID']);
                    print(
                        "project data value get${project.data}");
                    print(project.data["ACCTNAME"]);
                    setState(() {
                      provider.getContactEntity["ACCTNAME"] =
                      ...
                      builder: (BuildContext context) =>
                          AddRelatedEntityScreen(
                            ...
                            project: {
                              'ID': provider.getContactEntity['PROJECT_ID'],
                              'TEXT':
                              provider.getContactEntity['PROJECT_TITLE'],
                            },
                            account: provider.getContactEntity['ACCT_ID'] !=
                                null
                                ? {
                              'ID':
                              provider.getContactEntity['ACCT_ID'],
                              ...
                              provider.getContactEntity['ACCTNAME'],
                                ...
                                : {},
                            contact: provider.getContactEntity['CONT_ID'] !=
                                null
                                ? {
                              'ID':
                              provider.getContactEntity['CONT_ID'],
                              'TEXT':
                              '${provider.getContactEntity['FIRSTNAME'] ?? ''} ${provider.getContactEntity['SURNAME'] ?? ''}',
                            }
                                : {},
                            type: "opp",
                          ),
            ...
            body: Column(
              children: [
                Container(
                    height: 70,
                    child: CommonHeader(
                        entityType: widget.entityType, entity: provider.getContactEntity)),
                Container(
                  color: Colors.white,
                  height: 20,
                ),
  ...
  Future<void> handleOnNonDynamic(dynamic item) async {
    ...
    if (widget.type == "companies") {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) {
            return DynamicProjectNotes(
              project: _dynamicTabProvider.getContactEntity,
              typeNote: widget.type,
              notesData: response,
              isNewNote: false,
              entityType: widget.entityType,
    ...
    } else if (widget.type == "opp history") {} else
  ...
  Future<void> handleOnTapFunction(dynamic item) async {
    ...
    if (widget.type == "companies") {
      ...
      print(
          "deal id valeudifgc${_dynamicTabProvider.getContactEntity['DEAL_DEAL_ID']}");
      dynamic deal = await OpportunityService()
          .getEntity(item['DEAL_ID'].toString());
      Navigator.push(
        context,
