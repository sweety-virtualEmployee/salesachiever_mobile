...
class DynamicContactEditScreen extends StatefulWidget {
  ...
  const DynamicContactEditScreen({
    Key? key,
  ...
  @override
  State<DynamicContactEditScreen> createState() => _DynamicContactEditScreenState();
}

class _DynamicContactEditScreenState extends State<DynamicContactEditScreen> {
  String _notes = '';
  bool? _isNewNote;
  ...
  void _onChange(String key, dynamic value, bool isRequired) {
    setState(() {
      if (key == 'SITE_COUNTRY' &&  _dynamicTabProvider.getContactEntity[key] != value) {
         _dynamicTabProvider.getContactEntity['SITE_COUNTY'] = '';
      }

       _dynamicTabProvider.getContactEntity[key] = value;
    });
  }
  ...
  onRelatedValueSave(List<dynamic> entity) {
    entity.forEach((prop) {
      setState(() {
         _dynamicTabProvider.getContactEntity[prop['KEY']] = prop['VALUE'];
      });
    });
  }
  ...
  Widget generateFields(
    ...
    if (fieldData != null) {
      for (dynamic field in filedList) {
        print("field of entity${ _dynamicTabProvider.getContactEntity}");
          ...
          case 'C':
            if (field["FIELD_NAME"].contains("_ID")) {
              ...
              widgets.add(PsaRelatedValueRow(
                ...
                entity:  _dynamicTabProvider.getContactEntity,
  ...
  Future<void> saveProject() async {
    try {
      ...
      print("on saving${_dynamicTabProvider.getContactEntity}");
     if ( _dynamicTabProvider.getContactEntity['CONT_ID'] != null) {
          await ContactService().updateEntity( _dynamicTabProvider.getContactEntity['CONT_ID'],  _dynamicTabProvider.getContactEntity);
        } else {
          var newEntity = await ContactService().addNewEntity( _dynamicTabProvider.getContactEntity);
           _dynamicTabProvider.getContactEntity['CONT_ID'] = newEntity['CONT_ID'];
        }

        if (_isNewNote != null) {
          if (_isNewNote!) {
            await ContactService().addContactNote( _dynamicTabProvider.getContactEntity['CONT_ID'], _notes);
          } else {
            await ContactService()
                .updateContactNote( _dynamicTabProvider.getContactEntity['CONT_ID'], _notes)
                .onError((error, stackTrace) => null);
          }
        }
      _dynamicTabProvider.setCompanyEntity(  _dynamicTabProvider.getContactEntity);
      _dynamicTabProvider.setReadOnly(!_dynamicTabProvider.getReadOnly);
    } on DioError catch (e) {
      _dynamicTabProvider.setCompanyEntity( _dynamicTabProvider.getContactEntity);
